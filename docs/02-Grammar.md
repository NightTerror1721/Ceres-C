...
| Literals | Integers (decimal, octal with a leading `0`, `0x`, `0b`), floats (decimal and exponential), `char`, strings, `true`/`false`. Adjacent string literals are joined into one, as in C. Integer literals take C's suffixes - `u`/`U`, `l`/`L`, `ll`/`LL`, and `u` with either of the others in any order and case (`42ul`, `42LU`, `42ull`, `42llu`) - and get C's type: the first of their list in which the value fits (`int`, `long`, `long long` for a plain decimal; the unsigned types too for hex, binary and octal), with `int` and `long` 32 bits, so `4294967296` is a `long long` and `0xFFFFFFFF` an `unsigned int`. Floats are decimal (`1.5`, `1e3`) or hexadecimal with a binary exponent (`0x1.8p3` is 12; the `p` is required); they take an optional `f`/`F` or `l`/`L` (`long double`, which is `float` here, like `double`), and `f`/`F` also forces a digit run to be a float (`1f`). Character and string literals take C's prefixes: `L` (`wchar_t`, which is `int`), `u` (`char16_t`, `unsigned short`, UTF-16 with surrogate pairs), `U` (`char32_t`, `unsigned int`) and `u8` (UTF-8; a `u8` character is C23's `char8_t`, `unsigned char`, while a `u8` string keeps C17's `char` elements). A wide string is an array of its own element type, so it initializes an `int[]`, `unsigned short[]` or `unsigned int[]` and points to that element; the source is read as UTF-8, so `L"é"` is one `wchar_t`. When adjacent literals are joined, an unprefixed piece takes the prefix of the others (`"a" L"b"` is wide), and two different prefixes are an error. Escapes are C's: the named ones and `\?`, octal (`\101`, up to three digits), `\x` with every hex digit that follows (the value must fit one code unit), and the universal character names `\u` (four digits) and `\U` (eight). |

...
| `-Og` | Everything `-O1` does except the four that make the code harder to follow: no inlining, no frame-slot reuse (a slot per scope), no induction-variable pointer replacing a plain `base + i*C` with a hidden slot, and no byte fill/copy loop replaced by a call to a routine the unit carries. |

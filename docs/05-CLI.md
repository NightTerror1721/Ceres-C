...
| `--gc-sections` | Links the program with `ceres link --gc-sections`: the functions nothing reaches are left out of it, a library's included - a program that calls `strlen` no longer carries all of `string.c`. Used only with `--run`. |
...
| `-Og` | Everything `-O1` does except the four that make the code harder to follow: no inlining, no frame-slot reuse (a slot per scope), no induction-variable pointer replacing a plain `base + i*C` with a hidden slot, and no byte fill/copy/search loop replaced by a call to a routine the unit carries. |

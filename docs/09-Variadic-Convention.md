...
frame on entry, and `__builtin_va_arg` indexes into that area with a per-bank cursor, the way the
x86-64 SysV ABI does. That works, but it costs eight stores in the prologue of every variadic
function and three cursors in every `__builtin_va_list`.

Putting the tail on the stack instead makes it one contiguous run of words at a statically known
offset, which reduces `__builtin_va_list` to a single pointer and `__builtin_va_arg` to a load and
an add. AArch64 on Apple platforms makes the same trade for the same reason. The cost is that a
variadic call is slightly slower than an ordinary one — which is the right way round, since ordinary
calls are the common case and pay nothing.
...
tells the callee how big the copy is, so `__builtin_va_arg` could never read it back.
...
`__builtin_va_copy` are **built into the compiler**. There is no `<stdarg.h>` to include, and no
system include directory to find one in — the names are simply known:
...
| Form | What it means |
| --- | --- |
| `__builtin_va_list` | A cursor into the caller's frame. It is a `char*`, so it assigns, copies and passes like any other pointer. |
| `__builtin_va_start(ap, last)` | Points `ap` at the first argument of the tail. `last` must name the **last fixed parameter**. |
| `__builtin_va_arg(ap, T)` | Reads the next argument as `T` and advances `ap` one word. |
| `__builtin_va_end(ap)` | Nothing — a `__builtin_va_list` owns no resource. Write it anyway; it is free and it is what C expects. |
| `__builtin_va_copy(dst, src)` | Copies the cursor, so the tail can be walked twice. |

They are recognized as *syntax*, not as calls — `__builtin_va_arg`'s second operand is a type name,
which no ordinary call could express, and all four write through the `__builtin_va_list` the caller
named rather than through a copy of it. The names are not reserved: a program that uses
`__builtin_va_arg` as an ordinary variable keeps working, because only `__builtin_va_arg(` is
treated as the builtin.

### What the compiler checks

- `__builtin_va_start` is only allowed inside a function declared with `...`, and its second operand
  must name that function's last fixed parameter — naming any other one would describe a different
  starting point than the one `__builtin_va_start` actually produces.
- `__builtin_va_arg`'s type must be a 4-byte scalar. `__builtin_va_arg(ap, char)` is rejected rather
  than silently decoding a word that holds an `int`: after the default argument promotions, no
  `char` was ever passed. Read it as `int` and convert.
...
### Passing a `__builtin_va_list` on

Because a `__builtin_va_list` is an ordinary pointer, the `vprintf` pattern works: a variadic entry
point can hand its cursor to a non-variadic worker.
...
The worker advances its own copy of the cursor, not the caller's — the same as C, where a
`__builtin_va_list` passed by value may be consumed but the caller must not then use its own.
...
- [03-IR-to-CASM.md](03-IR-to-CASM.md) — the IR opcode behind `__builtin_va_start`, and the ordinary
  calling convention this one extends.

...
TEST(e2e, a_function_whose_frame_outgrows_a_16_bit_displacement_still_assembles_and_runs)
{
	...
	std::string body = "int main() { volatile unsigned int* term = (volatile unsigned int*)0xFF000004; int v0 = 1;";
	int value = 1;
	int middle = 0;
...
TEST(e2e, a_capped_float_width_computes_as_the_32_bit_type_it_really_is)
{
	...
	// it is a real 8-byte type now - F3.1b/F3.2 lower it, and the IR suite pins the guards that still
	// refuse the 64-bit operations F3.3-F3.4 have yet to add.)
...
TEST(e2e, adjacent_string_literals_print_as_one_string)
{
	...
	runsTheSameAtEveryLevel("string_concatenation",
		"void put(char c) { volatile unsigned int* t = (volatile unsigned int*)0xFF000004; *t = c; }"
